...
interface GameState {
  ...
  crtEffect: boolean;
  /** True once the persisted state has been rehydrated from localStorage (client only). */
...
export const useGameStore = create<GameState>()(
  persist(
    ...
    }),
    {
      ...
      // Next.js prerenders on the server; rehydrate explicitly on the client
      // (StoreHydrator) so the first render matches the server HTML.
      skipHydration: true,
